...
import type { SeedPickerPopup } from '../../src/game/ui/SeedPickerPopup.js';
import type { BuildMenuPanel } from '../../src/game/ui/BuildMenuPanel.js';
import type { InventoryPanel } from '../../src/game/ui/InventoryPanel.js';
import type { BuildingSystem } from '../../src/game/buildings/BuildingSystem.js';
import type { InventorySystem } from '../../src/game/items/InventorySystem.js';
import type { EquipmentState } from '../../src/game/tools/EquipmentState.js';
...
describe('BootSmoke (jsdom + stubbed canvas)', () => {
  ...
  it('runs the Phase 3 loop: picker, inventory panel, build mode placement', async () => {
    if (!game) {
      throw new Error('game was not created');
    }
    await waitFor(() => game?.scene.isActive('World') ?? false, 25000, 'World scene active');
    ...
    const phaserScene = game.scene.getScene('World') as Phaser.Scene;
    const toolbar = scene['toolbar'] as Toolbar;
    const seedPicker = scene['seedPicker'] as SeedPickerPopup;
    const buildMenu = scene['buildMenu'] as BuildMenuPanel;
    const inventoryPanel = scene['inventoryPanel'] as InventoryPanel;
    const buildings = scene['buildings'] as BuildingSystem;
    const inventory = scene['inventory'] as InventorySystem;
    const equipment = scene['equipment'] as EquipmentState;
    const isoRenderer = scene['isoRenderer'] as {
      renderedChunkCount: number;
      renderedObjectCount: number;
    };

    // Phase 3 textures were generated by Preload.
    for (const key of [
    ...
    // Seed picker: toolbar event opens it, picking equips seed:corn.
    toolbar.events.emit('seeds-requested', undefined);
    expect(seedPicker.visible).toBe(true);
    seedPicker.events.emit('seed-picked', { cropId: 'corn' });
    expect(seedPicker.visible).toBe(false);
    expect(equipment.get()).toMatchObject({ tool: ToolType.Seed, seedId: 'corn' });

    // Inventory panel toggles and starter stock is intact (minus 1 wheat seed).
    toolbar.events.emit('inventory-requested', undefined);
    expect(inventoryPanel.visible).toBe(true);
    expect(inventory.getQuantity('item:wood')).toBe(100);
    expect(inventory.getQuantity('item:stone')).toBe(60);
    expect(inventory.getQuantity('item:wheat_seed')).toBe(19);
    expect(inventory.getQuantity('item:wheat')).toBe(3); // harvested in the farming test
    toolbar.events.emit('escape-pressed', undefined);
    expect(inventoryPanel.visible).toBe(false);

    // Build mode: enter via toolbar, pick shed, rotate, place in the plot.
    toolbar.events.emit('build-requested', undefined);
    expect(scene['uiMode']).toBe(UiMode.Build);
    expect(buildMenu.visible).toBe(true);
    buildMenu.events.emit('building-picked', { buildingId: 'building:storage_shed' });
    toolbar.events.emit('rotate-requested', undefined);
    expect(scene['buildRotation']).toBe(90);

    const objectsBefore = isoRenderer.renderedObjectCount;
    const chunksBefore = isoRenderer.renderedChunkCount;
    const attemptPlace = scene['attemptPlace'] as (this: unknown, tile: { x: number; y: number }) => void;
    attemptPlace.call(scene, { x: 120, y: 150 });
    await sleep(200);
    expect(buildings.getPlaced()).toHaveLength(1);
    expect(buildings.getPlaced()[0]).toMatchObject({
      buildingId: 'building:storage_shed',
      anchorX: 119,
      anchorY: 149,
      rotation: 90,
    });
    // Shed cost deducted; exactly one object view added; no chunk rebuild.
    expect(inventory.getQuantity('item:wood')).toBe(70);
    expect(inventory.getQuantity('item:stone')).toBe(50);
    expect(isoRenderer.renderedObjectCount).toBe(objectsBefore + 1);
    expect(isoRenderer.renderedChunkCount).toBe(chunksBefore);

    // Esc exits build mode back to play.
    toolbar.events.emit('escape-pressed', undefined);
    expect(scene['uiMode']).toBe(UiMode.Play);
    expect(buildMenu.visible).toBe(false);
  }, 30000);
});
